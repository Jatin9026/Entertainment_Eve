  ...
  useEffect(() => {
    ...
    class Road {
      ...
      createPlane(side, width, isRoad) {
        ...
        // Determine the color and alpha for the plane based on whether it's road or island.
        // If the color is set to null, we want the mesh to be invisible.
        const specifiedColor = isRoad ? options.colors.roadColor : options.colors.islandColor;
        let color, alpha;
        if (specifiedColor == null) {
          // If null, use a dummy color (won't be visible) and set alpha to 0.
          color = new THREE.Color(0x000000);
          alpha = 0;
        } else {
          color = new THREE.Color(specifiedColor);
          alpha = 1;
        }
        ...
        let uniforms = {
          uTravelLength: { value: options.length },
          uColor: { value: color },
          uAlpha: { value: alpha }, // New uniform controlling opacity
          uTime: this.uTime
        };
    ...
    const roadBaseFragment = `
      ...
      varying vec2 vUv; 
      uniform vec3 uColor;
      uniform float uAlpha;  // New uniform controlling opacity
      uniform float uTime;
      ...
      void main() {
        ...
        // Use uAlpha instead of a hardcoded 1.0 for alpha
